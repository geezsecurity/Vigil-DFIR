  ...
  return new Promise((resolve, reject)=>{
    try{
      ...
      const doc = new PDFDocument({ size:"A4", margins:{ top:52, bottom:64, left:50, right:50 },
        info:{ Title:"DFIR Report — "+S(meta.caseName||meta.caseId,120), Author:"EVTX-Triage" }, bufferPages:true });
      ...
      facts([
        ...
        ["Prepared by", meta.preparedBy || (meta.analyst? meta.analyst+" · EVTX-Triage" : "EVTX-Triage (automated analysis)")],
      ]);
      gap(6);
      ...
      /* ---- disclaimer ---- */
      gap(10);
      small(S(R.disclaimer || "Generated by EVTX-Triage — automated triage aid; findings are heuristic and rule-based and require analyst validation. CONFIDENTIAL — handle per your organisation's data-classification policy.", 800));
      ...
      for(let i=0;i<range.count;i++){ doc.switchToPage(range.start+i);
        ...
        doc.text("EVTX-Triage — "+S(meta.caseId||"DFIR Report",60), L, y, {lineBreak:false});
...
export async function renderDocx(rawData){
  ...
  body.push(P(S(R.disclaimer || "Generated by EVTX-Triage — automated triage aid; findings are heuristic and rule-based and require analyst validation. CONFIDENTIAL — handle per your organisation's data-classification policy.",800),
    {color:GREY,size:15,before:120}));

  const doc = new Document({
    creator:"EVTX-Triage", title:"DFIR Report — "+S(meta.caseName||meta.caseId,120),
    styles:{ default:{ document:{ run:{ font:"Calibri" } } } },
    sections:[{
      properties:{ page:{ margin:{ top:900, bottom:1000, left:960, right:960 } } },
      footers:{ default: new Footer({ children:[ new Paragraph({ alignment:AlignmentType.CENTER, children:[
        run("EVTX-Triage — "+S(meta.caseId||"DFIR Report",60)+" · Page ", {size:15,color:GREY}),
